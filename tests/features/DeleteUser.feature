...
Background: 
Given a user exists in the database with email "joe123@mail.ca", username "joe123", password "pass123"

Scenario: Delete a user with email and username provided (Normal Flow)
When I make a DELETE request with the user information to "http://localhost:8000/users/user" with a valid username "joe123" and valid email "joe123@mail.ca"
...
When I make a DELETE request with the user information to "http://localhost:8000/users/user" with a valid username "joe123" and no email
Then the response status code for deletion should be 200
And the response for deletion should contain a status message "Success"
And the response for deletion should contain a message "User deleted successfully" 
